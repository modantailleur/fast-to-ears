...
def yamnet_frames_model_logmel(params, dm_input=None):
  """Defines the YAMNet logmel-to-class-scores model.

  Args:
    params: An instance of Params containing hyperparameters.
    dm_input: A dummy input log-mel spectrogram for features TypeSpec.

  Returns:
    A model accepting (num_spectrogram_frames, num_mel_bins) log-mel spectrogram input and emitting:
    - predictions: (num_patches, num_classes) matrix of class scores per time frame
    - embeddings: (num_patches, embedding size) matrix of embeddings per time frame
    - log_mel_spectrogram: (num_spectrogram_frames, num_mel_bins) spectrogram feature matrix
  """
  log_mel_spectrogram = layers.Input(batch_shape=(None,), dtype=tf.float32, tensor=dm_input)
  ...
  predictions, embeddings = yamnet(features, params)
  frames_model = Model(
      name='yamnet_frames_logmel', inputs=log_mel_spectrogram,
      outputs=[predictions, embeddings, log_mel_spectrogram])
  return frames_model
